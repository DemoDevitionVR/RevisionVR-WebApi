...
public class PositionService : IPositionService
{
    ...
    public async Task<IEnumerable<DeviceResultDto>> CreateAsync(DevicePosition dto)
    {
        var existDevice = await _appDbContext.Devices.FirstOrDefaultAsync(x => x.DeviceNumber.Equals(dto.DeviceNumber));
        if (existDevice is null)
            throw new DemoException(403, "Not found Device");

        var devices = _appDbContext.Devices.AsQueryable().Where(d => d.IsActive==true && d != existDevice);

        return this.mapper.Map<IEnumerable<DeviceResultDto>>(devices);
    }

    public async Task<IEnumerable<DeviceResultDto>> UpdateAsync(DevicePosition dto)
    {
        var existDevice = await _appDbContext.Devices.FirstOrDefaultAsync(x => x.DeviceNumber.Equals(dto.DeviceNumber));
        if (existDevice is null)
            throw new DemoException(403, "Not found Device");
